...
public class FollowPath : MonoBehaviour {
	...
	void Update () {
        ...
        ListOfStoredPositions[0].Add(LeaderSphere.position);
        LeaderSphere.position = Vector3.MoveTowards(LeaderSphere.position, target[Pathindex].position, step);
        ...
        for(int i = 0; i < FollowerSpheres.Length; i++)
        {
            ...
            if (ListOfStoredPositions[i].Count > 20 / speed)
            {
                ...
                if(i != FollowerSpheres.Length - 1)
                {
                    ...
                    ListOfStoredPositions[i + 1].Add(FollowerSpheres[i].position);
                }
                FollowerSpheres[i].position = ListOfStoredPositions[i][0];
                ListOfStoredPositions[i].RemoveAt(0);
            }
        }

        if (LeaderSphere.position == target[Pathindex].position)
        {
            Pathindex++;
        }
